...
public class EventResourceTest {
  ...
  @BeforeEach
  void setUp() {
    Event event = new Event(snakeId.toString(), null, "feed", LocalDate.of(2021, 10, 20), "jumper");
    jwt = generateJWT.generate("test@test.de", 60);

    when(eventService.getEventById(snakeId.toString(), uuid)).thenReturn(event);
    when(eventService.getEventById(snakeId.toString(), wrongId))
        .thenThrow(InternalServerErrorException.class);
    when(eventService.getEventsBySnakeId(snakeId.toString()))
  ...
  public void testDeleteSnakeById_error() {
    ...
    Assertions.assertEquals(
        "Was not able to delete the event", response.getBody().asPrettyString());
  }
}
